        ...
        <div class="col-12 col-md-4">
            <div class="form-group mb-2">
                ...
                <input type="number" class="form-control form-control-sm" id="house_no" name="house_no"
                    value="{{ old('house_no', $application->entityDetails->house_no ?? '') }}"
                    required>
            </div>
                ...
                <select class="form-select form-select-sm select2" id="state_id" name="state_id" required>
                    ...
                    @foreach($states as $state)
                        <option value="{{ $state->id }}"
                                {{ old('state_id', isset($application->entityDetails) ? $application->entityDetails->state_id : '') == $state->id ? 'selected' : '' }}>
                            {{ $state->state_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group mb-2">
                ...
                <div class="input-group input-group-sm">
                    <input type="text"
                        ...
                        id="email"
                        name="email"
                        value="{{ old('email', $application->entityDetails->email ?? '') }}"
                        required
...
<div class="row g-2 mb-3">
    <div class="col-md-4">
        ...
        <div id="pan_file_name" class="small text-muted {{ $panDoc ? '' : 'd-none' }} mt-1">
            @if($panDoc)
                <a href="#" data-bs-toggle="modal" data-bs-target="#documentModal" data-src="{{ Storage::disk('s3')->url('Connect/Distributor/pan/' . $panDoc['path']) }}" data-type="PAN Document">View</a> ({{ $panDoc['original_filename'] ?? $panDoc['path'] }})
                <input type="hidden" name="existing_pan_file" value="{{ $panDoc['path'] }}">
                <input type="hidden" name="existing_pan_file_original" value="{{ $panDoc['original_filename'] ?? $panDoc['path'] }}">
            @endif
        </div>
    </div>
    <div class="col-md-4">
...
<!-- GST Details -->
<div id="gst_fields" style="display: {{ old('gst_applicable', ($application->entityDetails->gst_applicable ?? ($gstDoc ? 'yes' : 'no'))) === 'yes' ? 'block' : 'none' }};" class="mb-3">
    <div class="row g-2">
        <div class="col-md-4">
            ...
            <div id="gst_file_name" class="small text-muted {{ $gstDoc ? '' : 'd-none' }} mt-1">
                @if($gstDoc)
                    <a href="#" data-bs-toggle="modal" data-bs-target="#documentModal" data-src="{{ Storage::disk('s3')->url('Connect/Distributor/gst/' . $gstDoc['path']) }}" data-type="GST Document">View</a> ({{ $gstDoc['original_filename'] ?? $gstDoc['path'] }})
                    <input type="hidden" name="existing_gst_file" value="{{ $gstDoc['path'] }}">
                    <input type="hidden" name="existing_gst_file_original" value="{{ $gstDoc['original_filename'] ?? $gstDoc['path'] }}">
                @endif
            </div>
        </div>
        <div class="col-md-4">
            ...
            <div class="input-group input-group-sm">
                <input type="text" 
                       ...
                       maxlength="15" 
                       title="GST must be 15 characters (e.g., 22AAAAA0000A1Z5)" 
...
<div class="row g-2 mb-3">
    <div class="col-md-4">
        ...
        <div id="seed_license_file_name" class="small text-muted {{ $seedLicenseDoc ? '' : 'd-none' }} mt-1">
            @if($seedLicenseDoc)
                <a href="#" data-bs-toggle="modal" data-bs-target="#documentModal" data-src="{{ Storage::disk('s3')->url('Connect/Distributor/seed_license/' . $seedLicenseDoc['path']) }}" data-type="Seed License Document">View</a> ({{ $seedLicenseDoc['original_filename'] ?? $seedLicenseDoc['path'] }})
                <input type="hidden" name="existing_seed_license_file" value="{{ $seedLicenseDoc['path'] }}">
                <input type="hidden" name="existing_seed_license_file_original" value="{{ $seedLicenseDoc['original_filename'] ?? $seedLicenseDoc['path'] }}">
            @endif
        </div>
    </div>
    <div class="col-md-4">
        ...
        <div class="input-group input-group-sm">
            <input type="text" 
                   ...
                   pattern="[A-Z0-9]{6,15}" 
                   maxlength="15" 
                   title="Seed License must be 6-15 alphanumeric characters" 
                   required 
...
<div class="row g-2 mb-3">
    <div class="col-md-2">
        ...
        <div id="bank_file_name" class="small text-muted {{ $bankDoc ? '' : 'd-none' }} mt-1">
            @if($bankDoc)
                <a href="#" data-bs-toggle="modal" data-bs-target="#documentModal" data-src="{{ Storage::disk('s3')->url('Connect/Distributor/bank/' . $bankDoc['path']) }}" data-type="Bank Document">View</a> ({{ $bankDoc['original_filename'] ?? $bankDoc['path'] }})
                <input type="hidden" name="existing_bank_file" value="{{ $bankDoc['path'] }}">
                <input type="hidden" name="existing_bank_file_original" value="{{ $bankDoc['original_filename'] ?? $bankDoc['path'] }}">
            @endif
        </div>
    </div>
    <div class="col-md-2">
                            ...
                            @foreach($authPersons as $index => $person)
                                ...
                                <td data-label="Letter of Authorisation">
                                                    ...
                                                    <div id="auth_person_letter_{{ $index }}_name" class="small text-muted mt-1 {{ isset($person['letter']) ? '' : 'd-none' }}">
                                                        @if(isset($person['letter']['path']))
                                                            <a href="#" data-bs-toggle="modal" data-bs-target="#documentModal"
                                                            data-src="{{ Storage::disk('s3')->url('Connect/Distributor/authorized_persons/' . $person['letter']['path']) }}"
                                                            data-type="Letter of Authorisation">View Letter</a>
                                                            {{--({{ $person['letter']['original_filename'] ?? $person['letter']['path'] }})--}}
                                                            <input type="hidden" name="existing_auth_person_letter[{{ $index }}]" 
                                                                value="{{ $person['letter']['path'] }}">
                                                            <input type="hidden" name="existing_auth_person_letter_original[{{ $index }}]" 
                                                                value="{{ $person['letter']['original_filename'] ?? $person['letter']['path'] }}">
                                                        @endif
                                                    </div>
                                                ...
                                                <td data-label="Aadhar Document">
                                                    ...
                                                    <div id="auth_person_aadhar_{{ $index }}_name" class="small text-muted mt-1 {{ isset($person['aadhar']) ? '' : 'd-none' }}">
                                                        @if(isset($person['aadhar']['path']))
                                                            <a href="#" data-bs-toggle="modal" data-bs-target="#documentModal"
                                                            data-src="{{ Storage::disk('s3')->url('Connect/Distributor/authorized_persons/' . $person['aadhar']['path']) }}"
                                                            data-type="Aadhar Document">View Aadhar</a>
                                                            {{--({{ $person['aadhar']['original_filename'] ?? $person['aadhar']['path'] }})--}}
                                                            <input type="hidden" name="existing_auth_person_aadhar[{{ $index }}]" 
                                                                value="{{ $person['aadhar']['path'] }}">
                                                            <input type="hidden" name="existing_auth_person_aadhar_original[{{ $index }}]" 
                                                                value="{{ $person['aadhar']['original_filename'] ?? $person['aadhar']['path'] }}">
                                                        @endif
                                                    </div>
