            ...
            @if ($checks->isNotEmpty() || $checkpointTypes->isNotEmpty() || (isset($supportingDocuments) && $supportingDocuments->isNotEmpty()))
                ...
                @if($coreTypes->isNotEmpty())
                    ...
                    @foreach($coreTypes as $type)
                        @php 
                            ...
                            $isAmount = $type === 'security_deposit' && is_numeric($files->first()?->reason ?? 0);
                        ...
                        @if($files->isNotEmpty())
                            <tr>
                                ...
                                <td>
                                    @if($isAmount)
                                        ₹{{ number_format($files->first()->reason, 2) }}
                ...
                @if($checkpointTypes->isNotEmpty())
                    ...
                    @foreach($checkpointTypes as $type)
                        @php 
                            $check = $groupedChecks->get($type, collect())->first() ?? (object)[
                                'received' => false,
                                'status' => 'pending',
                                'reason' => null
                            ];
                        ...
                        <tr>
                            <td>{{ $label }}</td>
                            <td><span class="badge bg-{{ $check->received ? 'success' : 'secondary' }}">{{ $check->received ? 'Yes' : 'No' }}</span></td>
                            <td><span class="badge bg-{{ $check->status === 'verified' ? 'success' : 'danger' }}">{{ $check->status === 'verified' ? 'Yes' : 'No' }}</span></td>
                            <td><span class="text-muted small">No upload required</span></td>
                            <td>{{ $check->reason && $check->status !== 'verified' ? $check->reason : 'N/A' }}</td>
                            <td>{{ $verifierName }}</td>
                            <td>{{ $verifiedDate }}</td>
                        </tr>
                    @endforeach
