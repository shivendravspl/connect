                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        @if($type === 'security_cheques')
                                            <div class="security-cheques-upload">
                                                ...
                                                @endforeach
                                                <button type="button" class="btn btn-sm btn-primary add-file-upload mt-1"><i class="fas fa-plus"></i> Add Cheque</button>
                                    ...
                                    <td class="align-middle">
                                        @if($type === 'security_deposit')
                                            <textarea name="documents[{{ $type }}][reason]" id="{{ $type }}_reason" class="form-control form-control-sm mb-1" rows="1" placeholder="Enter remarks if not verified">{{ old("documents.$type.reason", $check->reason) }}</textarea>
                                            <input type="number" name="security_deposit_amount" id="security_deposit_amount" class="form-control form-control-sm" value="{{ old('security_deposit_amount', $check->reason ?? '') }}" step="0.01" min="0" placeholder="Enter amount">
    ...
    <script>
        ...
        async function processDocument(file, fileNameField, existingFile, config) {
            try {
                ...
                // Hide replace button and show file input as d-none if needed, but since replaced, hide input
                const fileInput = fileNameField.closest('.file-upload-row')?.querySelector('.document-upload') || fileNameField.closest('td')?.querySelector('.document-upload');
                const replaceBtn = fileNameField.closest('.file-upload-row')?.querySelector('.replace-file-btn') || fileNameField.closest('td')?.querySelector('.replace-file-btn');
                ...
                const rowContainer = input.closest('.file-upload-row') || input.closest('td');
                let hiddenContainer = rowContainer ? rowContainer.querySelector('.hidden-file-container') : null;
                if (!hiddenContainer) {
        ...
        document.addEventListener('change', function(e) {
            if (e.target.matches('.document-upload')) {
                ...
                const fileNameField = document.getElementById(`file-name-${type}${type === 'security_cheques' ? '-' + input.closest('.file-upload-row').dataset.index : ''}`);
                const existingFile = input.closest('.file-upload-row')?.querySelector(`input[name="existing_${type}_file[]"]`)?.value || input.closest('td')?.querySelector(`input[name="existing_${type}_file"]`)?.value || '';
        ...
        document.addEventListener('click', function(e) {
            if (e.target.matches('.replace-file-btn')) {
                const row = e.target.closest('.file-upload-row') || e.target.closest('td');
                const fileInput = row.querySelector('.document-upload');
                const fileNameField = row.querySelector('.file-name');
                const hiddenContainer = row.querySelector('.hidden-file-container');
                const replaceBtn = row.querySelector('.replace-file-btn');
