...
class HomeController extends Controller
{
    ...
    public function dynamicData(Request $request)
    {
        try {
            ...
            // For MIS dashboard, we only need MIS applications
            if ($dashboardType === 'mis') {
                $data = $this->getMISData($filters, $user, $access_level, $request);
            ...
            // Regular dashboard - existing logic
            $data = $this->getDashboardData($filters, $user, $access_level, $request);
   ...
   protected function getMISData(array $filters, $user, $access_level, Request $request)
{
    // Helper lists (still useful for filters dropdowns)
    ...
    if ($filters['initiator_role']) {
        $misApplicationsQuery->where('core_employee.emp_designation', $filters['initiator_role']);
    }
    if ($filters['status']) {
        $misApplicationsQuery->where('onboardings.status', $filters['status']);
    }
    if ($filters['date_from'] && $filters['date_to']) {
        ...
        if ($filters['date_range_type'] === 'approval') {
            $misApplicationsQuery->whereExists(function ($q) use ($filters) {
                $q->select(DB::raw(1))
                  ->from('approval_logs')
            ...
            $misApplicationsQuery->whereBetween($dateField, [$filters['date_from'], $filters['date_to']]);
        }
    }

    $misApplications = $misApplicationsQuery->paginate(10);

    return compact('misApplications', 'bu_list', 'zone_list', 'region_list', 'territory_list');
}
    ...
    protected function getDashboardData(array $filters, $user, $access_level, Request $request)
    {
        ...
        if ($employee_details && $employee_details->isMisTeam()) {
            $misApplicationsQuery = Onboarding::query()
                ->with(['entityDetails', 'createdBy', 'currentApprover', 'territoryDetail', 'regionDetail', 'zoneDetail', 'approvalLogs'])
                ->whereIn('onboardings.status', ['draft', 'initiated', 'under_review', 'mis_processing', 'approved', 'document_verified', 'agreement_created', 'documents_received', 'distributorship_created', 'mis_rejected', 'physical_docs_verified','physical_docs_pending','documents_pending']);
